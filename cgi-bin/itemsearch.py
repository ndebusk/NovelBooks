...
if searchtype == "all":
    queryStringBook = "SELECT isbn, title, image FROM book"
    cursor.execute(queryStringBook)
    
elif searchtype == "genre":
    queryStringBook = "SELECT book.isbn, title, image FROM book, genre WHERE book.isbn = genre.isbn AND genre.genre = '" + searchterm + "'"
    cursor.execute(queryStringBook)
    
elif searchtype == "formatType":
    queryStringBook = "SELECT book.isbn, title, image FROM book, format WHERE book.isbn = format.isbn AND format.format = '" + searchterm + "'"
    cursor.execute(queryStringBook)
    
else:
    queryStringBook = "SELECT book.isbn, title, image FROM book WHERE (" + searchtype + " LIKE '%" + str(searchterm) + "%')"
    cursor.execute(queryStringBook)
