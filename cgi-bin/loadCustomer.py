...
for row in cursor:
    ...
    print '<p><form action="/cgi-bin/itementry.py" method="post"></p>'
    print '<p><input id="newpassword" type="password" name="newpassword" placeholder="Enter new password"/></p>'
    print '<p><input id="name" type="text" name="name" placeholder="' + str(row[1]) + '"/></p>'
    print '<p><input id="email" type="text" name="email" placeholder="' + str(row[2]) + '"/></p>'    
print '<input id="password" type="password" name="password" placeholder="Enter your current password to confirm new changes"/></p>'    
print '<button id="newUserSubmit" type="submit" class="btn btn-default">Save User Changes</button></p>'
print '</form>'
print '</div></div></div>'
