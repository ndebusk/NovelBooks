...
cursor = cnx.cursor()
queryStringUser = "SELECT userID, name FROM user WHERE username='" + str(user) + "'"
...
if (userID == -1):
    ...
    print "<table>"
    queryStringAddress = "SELECT street, city, state, zip FROM address WHERE userID='" + str(userID) + "'"
    cursor.execute(queryStringAddress)
    for row in cursor:
        print '<tr><td>' + row[0] + ', ' + row[1] + ' ' + row[2] + '</td><td><span id="stockspan">'
